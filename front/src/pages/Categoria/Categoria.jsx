...
const CategoriaCRUD = () => {
  ...
  const fetchCategorias = async () => {
    ...
    try {
      ...
      if (response && response.data && Array.isArray(response.data.content)) {
        ...
        setCategorias([]);
        console.warn(
          "Resposta da API de categorias em formato inesperado ou vazia:",
          response.data
        );
      }
    } catch (error) {
  ...
  const saveCategoria = async () => {
    ...
    if (categoria.nome.trim()) {
      const action = categoria.id
        ? categoriaService.alterar.bind(categoriaService)
        : categoriaService.inserir.bind(categoriaService);
      try {
        const response = await action(categoria);
  ...
  const deleteCategoria = async (id) => {
    try {
      const response = await categoriaService.excluir(id);
      await fetchCategorias();
      toast.current.show({
        severity: "success",
        summary: "Sucesso",
  ...
  const dialogFooter = (
    <div className="flex gap-2">
      <Button
        ...
        onClick={hideDialog}
      />
      <Button label="Salvar" icon="pi pi-check" onClick={saveCategoria} />
    </div>
  );
        ...
        <div className="flex flex-col gap-4">
          <div className="p-inputgroup flex-1">
            ...
            <InputText
              ...
              autoFocus
              className={`w-full ${
                submitted && !categoria.nome.trim() ? "p-invalid" : ""
              }`}
            />
          </div>
